...
export function simulatePriceDrop(id: string): { drop: number; price: number; alert: PriceAlert } | null {
  let result: { drop: number; price: number; alert: PriceAlert } | null = null
  alertsStore.set(list =>
    list.map(a => {
      ...
      const updated = {
        ...a,
        currentPriceGBP: next,
        lastDropAt: new Date().toISOString(),
        lastDropGBP: drop,
      }
      result = { drop, price: next, alert: updated }
      return updated
    }),
  )
  return result
