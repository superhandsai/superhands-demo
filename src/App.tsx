...
const FLIGHT_SEARCH_INPUT_STACKED =
  'relative w-full max-w-full min-w-0 h-auto min-h-[22px] flex-1 self-stretch p-0 m-0 text-[18px] leading-[1.25] overflow-hidden whitespace-nowrap text-ellipsis bg-transparent border-none focus:outline-none text-left'
const FLIGHT_SEARCH_INPUT_STACKED_STYLE = { color: 'rgba(33, 20, 27, 0.63)' }
...
export function AirportField({
  ...
  return (
    ...
    >
      ...
      <div
        ref={controlRef}
        className={`group relative flex flex-col items-stretch justify-center gap-[2px] flex-[0_0_auto] self-stretch w-full box-border min-h-20 px-[13px] py-[9px] border-2 rounded-[0px] bg-white cursor-pointer ${desktopSeam} ${mobileSwapClearance} ${openSeamClasses}`}
      >
        ...
        <div className={FLIGHT_SEARCH_VALUE_ROW}>
          ...
          <input
            ...
            style={!showHint ? FLIGHT_SEARCH_INPUT_STACKED_STYLE : undefined}
            role="combobox"
            aria-expanded={open}
...
export function DealCard({ image, title, highlight, price, description, imageClass }: DealCardProps) {
  ...
  return (
    <article className="group/card bg-white rounded-card max-md:rounded-none shadow-card overflow-hidden flex flex-col min-h-0 transition-[box-shadow,transform] duration-[220ms] ease-in-out hover:shadow-card-hover hover:-translate-y-[3px] motion-reduce:transition-none motion-reduce:hover:translate-y-0">
